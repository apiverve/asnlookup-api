...
class AsnlookupValidationException implements Exception {
  final List<String> errors;

  AsnlookupValidationException(this.errors);

  @override
  String toString() => 'AsnlookupValidationException: ${errors.join("; ")}';
}
...
/// ASN Lookup API client.
///
/// For more information, visit: https://apiverve.com/marketplace/asnlookup?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [asn] (required) - The ASN you want to get information about
class AsnlookupClient {
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'asn': ValidationRule(type: 'string', required: true),
  };

  AsnlookupClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/asnlookup',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [AsnlookupValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw AsnlookupValidationException(errors);
    }
  }

  /// Execute a request to the ASN Lookup API.
  ///
  /// Parameters are validated before sending the request.
  Future<AsnlookupResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw AsnlookupException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return AsnlookupResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw AsnlookupException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw AsnlookupException('Resource not found');
      } else {
        throw AsnlookupException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is AsnlookupException) rethrow;
      throw AsnlookupException('Request failed: $e');
    }
  }
...
/// Exception thrown by the ASN Lookup API client.
class AsnlookupException implements Exception {
  final String message;

  AsnlookupException(this.message);

  @override
  String toString() => 'AsnlookupException: $message';
}
